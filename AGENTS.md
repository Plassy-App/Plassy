...
- Local **PostgreSQL 16** and **Redis** are installed in the VM and started with `sudo pg_ctlcluster 16 main start` and `sudo redis-server --daemonize yes` (no systemd in the VM). DB/user: `plassy` / `plassy`, database `plassy`.
